...
export default function ServiceCard({ service }: { service: Service }) {
  ...
  return (
    <article className="group flex h-full flex-col rounded-2xl border border-navy-100 bg-white p-6 shadow-[var(--shadow-card)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[var(--shadow-card-hover)] dark:border-navy-800 dark:bg-navy-900/60 dark:shadow-none">
      <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-navy-950 text-white transition-colors duration-300 group-hover:bg-teal-600 dark:bg-white dark:text-navy-950 dark:group-hover:bg-teal-400">
        <Icon className="h-5 w-5" aria-hidden="true" />
      </div>
      <h3 className="mt-5 text-lg font-semibold text-navy-950 dark:text-white">{service.title}</h3>
